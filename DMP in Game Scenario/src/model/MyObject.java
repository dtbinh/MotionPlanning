...
public abstract class MyObject {
	...
	protected Configuration reference = null;
	...
	protected void setReferenceConfig(Scanner input) {
		StringTokenizer token = new StringTokenizer(ReadInNext(input));
		this.reference = new Configuration(Double.parseDouble(token.nextToken()),
	...
	public void show(Graphics g, boolean fill){
		if(fill){
			for(int i=0; i<getPlannerReferencePolygons().size(); i++){
				g.fillPolygon(Transformer.PlannerToCanvas(getPlannerReferencePolygons().get(i)));
			}
		}
		else{
			for(int i=0; i<getPlannerReferencePolygons().size(); i++){
				g.drawPolygon(Transformer.PlannerToCanvas(getPlannerReferencePolygons().get(i)));
			}
		}
	}
	...
	public boolean setReferenceConfig(double x, double y, double theta){
		return setReferenceConfig(new Configuration(x, y, theta));
	}
	
	public boolean setReferenceConfig(Configuration newConfig){
		this.reference = newConfig;
